// Verifica se o canvas está disponível globalmente
if (typeof canvas === 'undefined') {
    console.error('Canvas não encontrado. Certifique-se de que o canvas está definido no escopo global.');
} else {
    // Variáveis globais específicas para a funcionalidade de remoção de cor
    ...
    function openDetectionLevelModal() {
        ...
        if (modal) {
            ...
            console.error('Modal de detecção não encontrado.');
        }
    }
    ...
    const detectionLevelInput = document.getElementById('detection-level');
    if (detectionLevelInput) {
        detectionLevelInput.addEventListener('input', function () {
    ...
    const undoColorRemovalBtn = document.getElementById('undoColorRemovalBtn');
    if (undoColorRemovalBtn) {
        undoColorRemovalBtn.addEventListener('click', function () {
            if (lastRemovedState) {
